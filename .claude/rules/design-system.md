...
**Art from the mockup is downloaded, never approximated.** A mascot, a cover, an illustration:
export it and put it in the project. A rounded rectangle in roughly the right colour is not a
placeholder, it is a wrong screen that looks finished. Cannot get the file — say so and ask,
do not paint a stand-in.

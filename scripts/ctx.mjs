#!/usr/bin/env node
// ctx — сколько на самом деле стоит сессия.
//
// Читает транскрипт Claude Code (~/.claude/projects/<slug>/<session>.jsonl), где
// в каждом ответе лежит usage, и печатает факт, а не оценку:
//   BASE — окно на первом ответе: цена «просто открыть проект»
//   PEAK — максимум занятого окна за сессию
//   задачи — сколько добавила каждая
//   топ-потребители — какие вызовы инструментов съели окно
//
//   node scripts/ctx.mjs                  последняя сессия этого репо
//   node scripts/ctx.mjs --list           какие сессии есть
//   node scripts/ctx.mjs --session <id>   конкретная
//   node scripts/ctx.mjs --log "К0"       дописать строку в docs/ctx-log.md

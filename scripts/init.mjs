...
const mainTsx = () => (ds === 'xui'
  ? [
    ...
    "import { muteXuiSrcUnknownPropWarning, shouldForwardXuiSrcProp } from './kit/xui-source-meta'",
    "import { App } from './app'",
    '',
    ...
    '//',
    '// The style manager around it drops one prop on its way to the DOM: the mark the inspector',
    '// stamps on every call site. It has to ride the React element for the Context button to',
    '// find the component, and a library component that passes the rest of its props to a plain',
    '// tag hands it to the browser, where React calls it a typo. Every prototype had a console',
    '// full of that. Only this one prop is filtered; your own props on a styled wrapper go through.',
    'function Themed({ children }: { children: React.ReactNode }) {',
    '  const { theme } = useResolvedTheme({})',
    '  return (',
    '    <StyleSheetManager shouldForwardProp={shouldForwardXuiSrcProp}>',
    '      <ThemeProvider theme={theme}>{children}</ThemeProvider>',
    '    </StyleSheetManager>',
    '  )',
    '}',
    '',
    'muteXuiSrcUnknownPropWarning()',
    '',
